#!/usr/bin/env python3
"""
📈 CLAUDE GROWTH RATE TREND ANALYZER
====================================

Identifies trends with the HIGHEST GROWTH RATE in the last 3 days using Claude Opus 4.
...
class ClaudeGrowthTrendAnalyzer:
    """Analyze trends by growth rate using Claude Opus 4 directly"""
    
    def __init__(self, api_token: str):
        self.apify_client = ApifyContentIngestion(api_token)
        
        # Initialize Claude directly
        ...
        if anthropic_key and ANTHROPIC_AVAILABLE:
            ...
            print("⚠️ Claude not available. Set ANTHROPIC_API_KEY environment variable.")
    ...
    async def collect_daily_samples(self, day_offset: int, sample_size: int = 20):
        ...
        for hashtag in self.monitoring_hashtags[:10]:  # Sample from subset to avoid rate limits
            try:
    ...
    def calculate_growth_rates(self) -> List[Dict[str, Any]]:
        ...
        for trend in all_trends:
            ...
            if growth_score > 30:  # Lower threshold for detection
                trend_data = {
                    'trend': trend,
    ...
    async def get_claude_analysis(self, growth_trends: List[Dict]) -> str:
        """Get Claude Opus 4 analysis of growth trends"""
        if not self.claude_available or not growth_trends:
            return "Claude analysis not available or no trends to analyze."
        
        # Prepare top trends for analysis
        top_trends = growth_trends[:10]
        trends_summary = "\n".join([
            f"#{t['trend'].replace('phrase:', '').replace('format:', '').replace('cultural:', '')}: "
            f"Growth Score {t['growth_score']}, Stage: {t['trend_stage']}, "
            f"Timeline: {t['day_3_count']}→{t['day_2_count']}→{t['day_1_count']}"
            for t in top_trends
        ])
        
        # Get examples from videos
        examples = []
        for day_data in self.daily_data.values():
            examples.extend([v.description[:100] + "..." for v in day_data['videos'][:2]])
        
        examples_text = "\n".join(examples[:5])
        
        prompt = f"""
Analyze these EMERGING TikTok trends with highest growth rates in the last 3 days:
...
Provide analysis on:
1. **What's Exploding**: Which trends are in their explosive growth phase?
2. **Content Opportunities**: What should creators do RIGHT NOW to catch these trends?
3. **Trend Predictions**: Which of these will continue growing vs peak soon?
4. **Creator Strategy**: Specific actionable advice for different types of creators
5. **Timing**: How much time do creators have to jump on each trend?

Focus on ACTIONABLE insights for creators who want to ride the fastest-growing trends before they peak.
"""
    ...
    def display_growth_analysis(self, growth_trends: List[Dict], claude_insights: str):
...
async def main():
    ...
    print("📈 CLAUDE GROWTH RATE TREND ANALYZER")
    print("=" * 45)
    print("🎯 Finding trends with HIGHEST GROWTH in last 3 days...")
    print("🤖 Using Claude Opus 4 for analysis!")
    print()
    
    analyzer = ClaudeGrowthTrendAnalyzer(api_token)
    
    print("📊 COLLECTING 3-DAY DATA SAMPLE")
    print("=" * 35)
    
    # Collect data for each day (reduced sample size for speed)
    for day in range(3):
        success = await analyzer.collect_daily_samples(day, sample_size=15)
        if not success:
    ...
    # Get Claude analysis
    print("\n🤖 Getting Claude Opus 4 analysis...")
    claude_insights = await analyzer.get_claude_analysis(growth_trends)
    
    # Display results
    analyzer.display_growth_analysis(growth_trends, claude_insights)
